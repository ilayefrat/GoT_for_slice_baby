...
bool populate_eviction_set(eviction_set *llc_eviction_set, uintptr_t evictee) {
    ...
    if (reduce_eviction_set(sentinel, INITIAL_SAMPLE_SIZE, evictee, CACHE_ASSOCIATIVITY)) {
...
bool populate_eviction_set_l2(eviction_set *l2_eviction_set,
                              uintptr_t evictee) {
  node_array candidates = {0};
  if (eviction_set_memory == NULL) {
    initialize_allocation(&eviction_set_memory, ALLOCATION_SIZE);
  ...
  node_array_to_linked_list(candidates, sentinel);
  node_array _l2_eviction_set = {0};

  free(candidates.arr);

  if (reduce_eviction_set(sentinel, INITIAL_SAMPLE_SIZE, evictee,
                          L2_CACHE_ASSOCIATIVITY)) {
    _l2_eviction_set.length = L2_CACHE_ASSOCIATIVITY;
    _l2_eviction_set.arr = malloc(_l2_eviction_set.length * sizeof(node *));
    linked_list_to_node_array(sentinel->next, _l2_eviction_set);

    l2_eviction_set->arr = (uintptr_t *)_l2_eviction_set.arr;
    l2_eviction_set->length = _l2_eviction_set.length;
