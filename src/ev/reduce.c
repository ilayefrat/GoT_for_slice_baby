...
bool reduce_eviction_set_rec(node *sentinel, uint32_t length, uintptr_t evictee, uint32_t target_size);

bool reduce_eviction_set(node *sentinel, uint32_t length, uintptr_t evictee, uint32_t target_size) {
    backtrack_count = 0;
    return reduce_eviction_set_rec(sentinel, length, evictee, target_size);
}
